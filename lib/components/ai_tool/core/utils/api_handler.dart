...
class ApiHandler {
  // class to manage the api interactions
  ...
  ApiHandler() {
    ...
    // if the api key is not found in the environment, fall back to the AI api key constant
    final apiKey =
        Platform.environment['API_KEY'] ?? AI_API_KEY; // obtain the api key

    // initialize the generative model with the specified model name and api key
    // the model name is 'AI-1.5-flash', and the api key is used for authentication
    _model = GenerativeModel(
        model: 'AI-1.5-flash', apiKey: apiKey); // initialize the model
  } // end of constructor
  ...
  Future<String> generateResponse(String inputText) async {
    ...
    return response.text ??
        'No response generated.'; // return the generated response or a default message
